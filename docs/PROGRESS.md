...
- **Active: Phase 10** (post-playtest polish/optimization) — 10.1–10.6 DONE; next **10.7 all-cards-owned softlock**; then 10.8; **10.9 optimization pass LAST**. **Phase 11 — multiplayer/co-op** spec'd 2026-08-21 + Q1–Q7 answered, docs only, queued after Phase 10; open: **11.13 web-play research** (binding protocol + 100%-free, PLAN §3.8). **Phase 12 — flame tune + 3 new weapons / 4 new synergies** spec'd 2026-08-21, queued after Phase 11. **Phase 13 — 3-map expansion (Higan + The Drowned City) + level select + mobile view zoom** spec'd 2026-08-21; **A1–A8 answered 2026-08-21** (skins approved, crab, M03 = 1.5× area ≈5145×3920, chained ×1.25 difficulty, A5 flavor approved, zoom 0.80 + Pause-menu Settings w/ mute replacement, names tentatively approved). Order per A7: **10.7/10.8 → 13 → 11 → 12 → 10.9 (LAST, D25) → 2.9**. Deferred (NOT building): first-clear bonus, shard multipliers, per-level victory line, pufferfish, keyboard 1/2/3 select, boss intro moment.
...
- **Overall:** Phases 0–9 done (published); after **10.7/10.8** → **Phase 13 multi-level** → **Phase 11 multiplayer** → **Phase 12 weapons** → **10.9 optimization (LAST, D25)** → **2.9 user browser sign-off** (re-verify post-co-op) → project DONE. Blocking: none.
...
- **Git:** tree clean — Phase 9 + 10.1–10.6 + Phase 13 spec committed + pushed 2026-08-21 (10.5 = `af6cd5d`, 10.6 = `c326004`, Phase 13 spec = `1f7d8d6`). Pages auto-deploys from main within a few minutes. Commit only if the user asks (AGENTS rule 7).
...
### Phase 10 — Post-playtest polish & optimization (active — 10.7/10.8 now; 10.9 deferred to LAST per D25/A7)
...
- [ ] 13.1 Level framework: `js/world/levels.js` (pure defs: size/seed/palette/layout hook/weights/boss/particles/audio token/**per-level difficulty scalar (A4)**) + `generateWorld(seed, levelKey)` (Map 01 output identical) + per-level minimap + **per-level menu backdrop preview (A5)**
- [ ] 13.2 Map 02 "Higan" art: cherry trees, **sakura petals (Map 02 only — replace falling snow)**, torii/pagoda/shrine/stone-lantern/bamboo/koi pond (A5)/stone paths, Fuji silhouette, dusk-pink palette + full moon, torii-ring landmark
- [ ] 13.3 Map 02 roster: **Oni** (brute) + **Ryū dragon** (boss; segmented tail A5) — user spec + approved skins **tanuki / hō-ōi / shikome / kitsune / miko** (A1) + per-level stat tables (×1.25 difficulty, A4) + weights
...
Order: 10.7 → 10.8 in listed order (each: implement → all three gates green → tick with date + one-liner); **10.9 runs LAST** so it measures the final state. Facts re-verified against code 2026-08-21 (gates green; no `gem` sfx recipe; no empty-pool guard in game.js). **Phase 13 (3-map expansion) spec'd in PLAN §3.9 — A1–A8 answered 2026-08-21; begins after 10.7/10.8, then Phase 11 (A7 order); deferred (NOT building, D51): first-clear bonus, shard multipliers, per-level victory line, pufferfish, keyboard 1/2/3 select, boss intro moment.**
...
| 53 | Meta progression (Soulshards + Upgrades, `qsurv.meta.v1`) is **player-specific**: never carried over to, shared with, or applied to other players during co-op sessions; each client applies its own local meta to its own character; run shard earnings accrue only to the local player's LS; host sim receives each player's meta-derived stat profile at join (handshake) for accurate simulation — sim input only, no meta in per-step snapshots | User spec (2026-08-21) |
...
2. **Active: Phase 10 → resume at 10.7** (10.7/10.8 in order; **10.9 runs LAST** — after 13/11/12, per D25/A7). Then → **Phase 13 multi-level** (PLAN §3.9; A1–A8 answered; deferred items in D51 — do not build) → **Phase 11 multiplayer** (spec complete; room handshake carries the level key; 11.13 web-play research follows the binding protocol in PLAN §3.8) → **Phase 12 weapons** (flame tune + 3 new weapons + 4 new synergies) → **10.9 optimization (LAST)** → **2.9** user browser sign-off (re-verify post-co-op) → tick 2.9 → project DONE. New scope → add to the Master Checklist first.
