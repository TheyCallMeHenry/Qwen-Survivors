...
- **Active: Phase 13** (multi-level) — **13.1 + 13.2 DONE 2026-08-21** (level framework + Map 02 "Higan" art: layoutM02 torii-ring village + cherry/bamboo/koi-pond/sakura-petal art, palette-driven sky/ground/lighting/foreground; m01 bit-identical). Next: **13.3 M02 roster** (Oni + Ryū boss + skins + stat tables ×1.25 + weights). **Phase 11 — multiplayer/co-op** spec'd + Q1–Q7 answered, docs only, queued after Phase 13; open: **11.13 web-play research** (binding protocol + 100%-free, PLAN §3.8). **Phase 12 — flame tune + 3 new weapons / 4 new synergies** spec'd, queued after Phase 11. **Phase 10:** 10.1–10.8 done; **10.9 optimization pass** runs **LAST**. Order per A7: **13 → 11 → 12 → 14 → 10.9 (LAST, D25) → 2.9**. Deferred (NOT building): first-clear bonus, shard multipliers, per-level victory line, pufferfish, keyboard 1/2/3 select, boss intro moment.
- **Gates (all three green before any tick):** `node tools/check.mjs` **30/30** · `node tools/test-logic.mjs` **243/243** · `node tools/test-boot.mjs` **`PASS boot-sim`**.
...
- **Git:** tree clean — 13.2 Map 02 "Higan" art committed + pushed 2026-08-21 (13.2 = `0e2d601`). Pages auto-deploys from main within a few minutes. Commit only if the user asks (AGENTS rule 7).
...
## Resume Notes — next: Phase 13 — 13.3 Map 02 roster (start here)

**13.3 M02 roster (scope = checklist 13.3 line):** Oni (brute slot) + **Ryū dragon boss** (segmented tail, A5) + approved skins **tanuki/hō-ōi/shikome/kitsune/miko** (A1) re-skinning the base roster (mechanics unchanged) + per-level stat tables (×1.25, A4) + m02 `weights` → wire `boss` + `weights` in levels.js (unblocks a REAL m02 run in test-boot). m02 art/layout already live (13.2): `generateWorld(seed,'m02')` golden 274 trees / 324 decor / 331 colliders (seed 20260822, pinned).

Phase 10 done except **10.9 (runs LAST — after 13/11/12 per D25/A7 — so it measures the final state)**. Phase 13: **13.1 + 13.2 DONE 2026-08-21** (framework + Higan art; m02 `weights`/`boss` + m03 `layout`/`weights`/`boss` still null — land with 13.3–13.5; m03 palette final values land 13.4). Next: **13.3 M02 roster** (PLAN §3.9; deferred items D51 — do not build). **Phase 14 (in-run Soulshards counter + unified co-op earnings)** spec'd + **O1 resolved (A: live projected award `floor(score/400)` +25 victory, no economy change)** — queued after 12, before 10.9 (placement approved 2026-08-21).
...
2. **Active: Phase 13 multi-level** (PLAN §3.9; A1–A8 answered; 13.1 done — begins **13.2 Higan art**; deferred items in D51 — do not build) → **Phase 11 multiplayer** (spec complete; room handshake carries the level key; 11.13 web-play research follows the binding protocol in PLAN §3.8) → **Phase 12 weapons** (flame tune + 3 new weapons + 4 new synergies) → **Phase 14 in-run shard counter** (14.1–14.3) → **10.9 optimization (LAST)** → **2.9** user browser sign-off (re-verify post-co-op) → tick 2.9 → project DONE. New scope → add to the Master Checklist first.
