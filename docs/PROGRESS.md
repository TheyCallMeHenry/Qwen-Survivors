...
- **Active: Phase 13** (multi-level) — **13.1–13.8 DONE** (framework + Higan art + M02 roster + M03 art + M03 roster + unlock persistence `qsurv.wins.v1` + **level select: 3 radio cards, locked-visible + denied blip/shake, backdrop preview, persist `qsurv.level.v1`** + **view zoom 0.80/1.0 (touch default 0.80, `qsurv.zoom.v1`) + Pause-menu Settings (zoom + mute; mute replaced HUD `#btn-mute`)**. Next: **13.9 per-level high scores** (one LS list per level; existing `qsurv.hiscores.v1` = M01 — no data loss). **Phase 11 — multiplayer/co-op** spec'd + Q1–Q7 answered, docs only, queued after Phase 13; open: **11.13 web-play research** (binding protocol + 100%-free, PLAN §3.8). **Phase 12 — flame tune + 3 new weapons / 4 new synergies** spec'd, queued after Phase 11. **Phase 10:** 10.1–10.8 done; **10.9 optimization pass** runs **LAST**. Order per A7: **13 → 11 → 12 → 14 → 10.9 (LAST, D25) → 2.9**. Deferred (NOT building): first-clear bonus, shard multipliers, per-level victory line, pufferfish, keyboard 1/2/3 select, boss intro moment.
...
## Resume Notes — next: Phase 13 — 13.9 per-level high scores (start here)

**13.9 Per-level high scores (scope = checklist 13.9 line, A5):** one LS list **per level** — existing `qsurv.hiscores.v1` IS the M01 list (no data loss; keep that key for m01), m02/m03 get sibling keys (document the naming when implementing); `ui/screens.js` scores screen shows the **selected level's** list (it already reads `CFG.scores.storageKey` once — make it per-level); game over `rankScore`/`saveScores` rank within **the run's level** list (run's level = `game.levelKey`). `loadScores`/`saveScores` in screens.js are already pure + key-parametric. Add test-logic block (per-level keys, m01 untouched) + test-boot E2E (scores saved separately per level) before ticking.

Phase 10 done except **10.9 (runs LAST — after 13/11/12 per D25/A7 — so it measures the final state)**. Phase 13: **13.1–13.8 DONE** (framework + Higan art + M02 roster + M03 roster + unlock persistence + level select + zoom/Settings). Next: **13.9 per-level high scores** (PLAN §3.9; deferred items D51 — do not build).
Phase 10 done except **10.9 (runs LAST — after 13/11/12 per D25/A7 — so it measures the final state)**. Phase 13: **13.1–13.7 DONE** (framework + Higan art + M02 roster + M03 art + M03 roster + unlock persistence + level select). Next: **13.8 mobile view zoom + Settings** (PLAN §3.9; deferred items D51 — do not build). **Phase 14 (in-run Soulshards counter + unified co-op earnings)** spec'd + **O1 resolved (A: live projected award `floor(score/400)` +25 victory, no economy change)** — queued after 12, before 10.9 (placement approved 2026-08-21).
...
2. **Active: Phase 13 multi-level** (PLAN §3.9; A1–A8 answered; 13.1–13.8 done — begins **13.9 per-level high scores**; deferred items in D51 — do not build) → **Phase 11 multiplayer** (spec complete; room handshake carries the level key; 11.13 web-play research follows the binding protocol in PLAN §3.8) → **Phase 12 weapons** (flame tune + 3 new weapons + 4 new synergies) → **Phase 14 in-run shard counter** (14.1–14.3) → **10.9 optimization (LAST)** → **2.9** user browser sign-off (re-verify post-co-op) → tick 2.9 → project DONE. New scope → add to the Master Checklist first.
