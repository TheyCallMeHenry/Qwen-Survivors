...
- **Active: Phase 13** (multi-level) — **13.1–13.4 DONE 2026-08-21** (framework + Higan art + M02 roster + M03 art: layoutM03 5145×3920, kelp/Atlantis city/coral/god-rays/rising bubbles/fish schools, m03 palette final values). Next: **13.5 M03 roster** (crab/goldfish/mer-people/stingray/eel/orca + Great White Shark boss; ×1.56 stat tables; m03 weights + boss wiring). **Phase 11 — multiplayer/co-op** spec'd + Q1–Q7 answered, docs only, queued after Phase 13; open: **11.13 web-play research** (binding protocol + 100%-free, PLAN §3.8). **Phase 12 — flame tune + 3 new weapons / 4 new synergies** spec'd, queued after Phase 11. **Phase 10:** 10.1–10.8 done; **10.9 optimization pass** runs **LAST**. Order per A7: **13 → 11 → 12 → 14 → 10.9 (LAST, D25) → 2.9**. Deferred (NOT building): first-clear bonus, shard multipliers, per-level victory line, pufferfish, keyboard 1/2/3 select, boss intro moment.
- **Gates (all three green before any tick):** `node tools/check.mjs` **30/30** · `node tools/test-logic.mjs` **269/269** · `node tools/test-boot.mjs` **`PASS boot-sim` (runs=3)**.
- **Overall:** Phases 0–9 done (published) + 10.1–10.8 + 13.1–13.4 done; next **Phase 13 (13.5 M03 roster)** → **Phase 11 multiplayer** → **Phase 12 weapons** → **10.9 optimization (LAST, D25)** → **2.9 user browser sign-off** (re-verify post-co-op) → project DONE. Blocking: none.
...
- **Git:** tree clean — 13.4 M03 art committed + pushed 2026-08-21 (13.4 = `a8c332b`). Pages auto-deploys from main within a few minutes. Commit only if the user asks (AGENTS rule 7).
...
## Resume Notes — next: Phase 13 — 13.5 M03 roster (start here)

**13.5 M03 roster (scope = checklist 13.5 line):** slot re-skins in `buildCharacters('m03')` (m02 pattern) — **crab** (small chaser), **goldfish** (bat), **mer-people** (goblin; merman + mermaid skins), **stingray** (wolf), **electric eel** (cultist; jagged lightning-zap sprite, NOT orb), **orca** (brute), **Great White Shark** (boss; same charge skeleton, belly-flash windup) + m03 `weights` (m01 role-curve pattern) + boss `@240` with banner name + stat tables (×1.56 vs M01 via existing per-level `diff`, A4) + real **M03 run 4** in test-boot mirroring the 13.3 M02-run-3 pattern (keep-alive + banner capture + victory). m03 `weights`/`boss` still `null` in levels.js — land here. m03 audio token 'drowned' lands 13.11.

Phase 10 done except **10.9 (runs LAST — after 13/11/12 per D25/A7 — so it measures the final state)**. Phase 13: **13.1–13.4 DONE 2026-08-21** (framework + Higan art + M02 roster + M03 art; m03 `weights`/`boss` still null — land with 13.5). Next: **13.5 M03 roster** (PLAN §3.9; deferred items D51 — do not build). **Phase 14 (in-run Soulshards counter + unified co-op earnings)** spec'd + **O1 resolved (A: live projected award `floor(score/400)` +25 victory, no economy change)** — queued after 12, before 10.9 (placement approved 2026-08-21).
