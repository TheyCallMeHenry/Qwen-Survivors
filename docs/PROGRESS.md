...
- **Active: Phase 13** (multi-level) — **13.1–13.6 DONE 2026-08-21** (framework + Higan art + M02 roster + M03 art + M03 roster + unlock persistence: `qsurv.wins.v1` per-level victory counts, victory-only, wired into game-over). Next: **13.7 level select** (3 cards, locked-visible + denied blip/shake, ≥72 px touch, persist last selection). **Phase 11 — multiplayer/co-op** spec'd + Q1–Q7 answered, docs only, queued after Phase 13; open: **11.13 web-play research** (binding protocol + 100%-free, PLAN §3.8). **Phase 12 — flame tune + 3 new weapons / 4 new synergies** spec'd, queued after Phase 11. **Phase 10:** 10.1–10.8 done; **10.9 optimization pass** runs **LAST**. Order per A7: **13 → 11 → 12 → 14 → 10.9 (LAST, D25) → 2.9**. Deferred (NOT building): first-clear bonus, shard multipliers, per-level victory line, pufferfish, keyboard 1/2/3 select, boss intro moment.
- **Gates (all three green before any tick):** `node tools/check.mjs` **30/30** · `node tools/test-logic.mjs` **286/286** · `node tools/test-boot.mjs` **`PASS boot-sim` (runs=4)**.
- **Overall:** Phases 0–9 done (published) + 10.1–10.8 + 13.1–13.6 done; next **Phase 13 (13.7 level select)** → 13.8–13.13 → **Phase 11 multiplayer** → **Phase 12 weapons** → **10.9 optimization (LAST, D25)** → **2.9 user browser sign-off** (re-verify post-co-op) → project DONE. Blocking: none.
...
- **Git:** tree clean — 13.5 M03 roster + 13.6 unlock persistence committed + pushed 2026-08-21 (13.5+13.6 = `39551c3`). Pages auto-deploys from main within a few minutes. Commit only if the user asks (AGENTS rule 7).
...
## Resume Notes — next: Phase 13 — 13.7 level select (start here)

**13.7 Level select (scope = checklist 13.7 line):** 3 cards (radio) on main menu + Start begins selection; locked levels visible + non-selectable + requirement text + X/3 progress from `game.wins` via `isUnlocked`/`recordWin` helpers (13.6, in meta.js); **denied blip + card shake** on locked tap (A5 — new SFX blip + CSS animation, both procedural); ≥72 px touch targets + safe areas (mobile parity); persist last selection (LS, `qsurv.*` pattern — likely `qsurv.level.v1` in CFG). Level select UI = `ui/screens.js` (main menu is there); cards should show per-level name + backdrop hint (13.2/13.4 menu-backdrop hook already previews last-level). m03 audio token 'drowned' still lands 13.11.

Phase 10 done except **10.9 (runs LAST — after 13/11/12 per D25/A7 — so it measures the final state)**. Phase 13: **13.1–13.6 DONE 2026-08-21** (framework + Higan art + M02 roster + M03 art + M03 roster + unlock persistence). Next: **13.7 level select** (PLAN §3.9; deferred items D51 — do not build). **Phase 14 (in-run Soulshards counter + unified co-op earnings)** spec'd + **O1 resolved (A: live projected award `floor(score/400)` +25 victory, no economy change)** — queued after 12, before 10.9 (placement approved 2026-08-21).
