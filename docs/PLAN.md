...
- **RESOLVED 2026-09-07 (D84, user binding):** skip-XP = **66% of the next level's requirement** · banish by **card key**, VS-fidelity **owned-freeze** (banished owned card keeps rank, never re-offered/upgradable this run) · reroll exclusion = **that redraw only** · **10 shop levels each** (level 1 = unlock = 1 use, +1/level, cap 10 uses/run; supersedes the 5 cap in D67) at tiers **[300,600,…,3000]** · economy slow-down: the 5 existing meta-upgrade curves → **[50,100,200,400,1000]** · UI: 3 buttons + live per-player use counts, ≥72 px, exact-effect text (10.2). 10.7/15.3 interplay: banish/reroll draws run through the same pool + uniform draw; empty-pool auto-skip unchanged.
...
### 3.14 Playtest defects round 3 + projectile-feel pass (user report 2026-09-04; six items → Phase 22.5–22.7 + new Phase 23; **scoping-only this turn — no implementation**)

User tested the published Pages build. Six asks, scoped below against in-tree code (facts verified, not assumed). Standing instruction recorded: **no judgement-based weapon tuning beyond what the user explicitly names**; ask before touching any projectile weapon's cadence they did not name.

**(1) Synergy gating defect — CONFIRMED BUG, not a perception issue.** `cardOffers` (`js/entities/player.js`) gates every synergy on `S.requires.every((r) => reqAtMax(weapons, passives, r))`, and `reqAtMax` requires each named source at **its own** max. The user's report is close but the actual rule is narrower than "all weapons": only the synergy's **own two sources** must be maxed. Because a run caps weapon slots (5 solo) and every offered synergy needs its specific pair at L5, a player holding several mid-level weapons sees **zero** synergy cards for long stretches — the symptom reads as "nothing until everything is maxed." Two further effects make it worse: `cardOffers` draws 3 from a pool that contains no synergies at all until a pair completes, and the pre-11.5 weapon-cap means pairs are rarely both maxed early. **Scope = 22.5** (defect-class, Phase 22): decide whether this is intended design (it matches PLAN §3 / Phase 12 "offered the moment their own 2 sources max") and therefore a UX/legibility problem instead — surface synergy readiness in the level-up UI (e.g. a near-ready pair hint) rather than changing the gate. **Needs a user decision: change the gate, or keep it and improve legibility?** Do not silently re-gate.

**(2) Projectile attack-speed variation + Bow charge-up.** Verified current cadences (L1 → L5, seconds between volleys): Wand `rate` 0.60→0.32 · Twin Fangs `rate` 0.55→0.30 · Bow `rate` **identical to Twin Fangs** 0.55→0.30 · Axe `cd` 1.40→0.80 · Bombs `cd` 2.2→1.4. The Bow and the pistols are **numerically identical in cadence**, which is exactly the "doesn't feel unique" report. User-named changes only: (a) Bow gains a brief **charge-up** wind-up (drawn-string visual, new draw state + `CFG.weapons.bow.charge` scalar) and its interval increases slightly; (b) Wand interval increases very slightly under the constraint `pistols < wand < bow`; (c) pistols / axe / bombs explicitly **leave as-is**. Ordering constraint to satisfy at every level: `bow.rate(L) > wand.rate(L) > pistols.rate(L)`. **Scope = 23.1** (new Phase 23 — projectile feel pass, spec'd 2026-09-04). Unnamed projectile weapons (Sunder Bombs cadence is named-as-fine; the flame is not interval-driven) → **ask the user, do not adjust.**

**(3) New synergy: Heart of Oak × Lodestone.** Verified `pickups.js` — hearts have **no magnet logic whatsoever** (only gems test `d < magR` and set `g.mag`); the heart loop is collect-radius only. So this is a genuine new mechanic, not a flag flip. Proposal: `CFG.synergies.<key>` requires `['hp','magnet']` at max → hearts within `magR = CFG.gems.magnetBase × player.magnet` ride the existing gem magnet flight path (same `sp` ramp, snapshot-neutral — pickups already ride `pickupSnaps`). Note **naming collision to resolve with the user:** PLAN Phase 12 already reserves a synergy on max-HP (`Heart-Piercer` = Bow + max-HP passive), so two hp-based synergies will coexist — confirm both are wanted and pick non-conflicting keys/icons. **Scope = 23.4 — RESOLVED 2026-09-05 (user scoped it in): Phase 23 shipped without it (23.4 became the gates step), so it lands as the 5th new synergy in `12.6` (key `heartMagnet`, 5 levels scaling the heart-magnet radius), covered by the 12.7 pool/README sync + 12.8 gates.**

**(4) Bomb blast radius ×1.5 + strong radial push-back + visual must match.** Verified: `bombKb` is **already applied radially from the blast centre** (`damageEnemy(e, b.dmg, dx/d, dy/d, C.bombKb, …)`), so "originating from the center" exists — the ask is a magnitude increase. Per-level radii are 95/105/115/128/140 → ×1.5 = **142/157/172/192/210**. **Visual defect confirmed:** `combat.draw` scales the blast sprite as `s = x.r * 2 * (0.6 + 0.6k)`, i.e. drawn radius runs **0.60× → 1.20×** the damage radius, so for the first third of the flash the blast is visibly **smaller** than the area it damages — precisely the mismatch reported. Fix = drive the sprite from the true radius (start at/above 1.0× and fade out) rather than the current grow-from-small ramp. `explosionSprite` gradient reaches ~0.97 of drawn half-width, so a 1.0× floor is honest. **Scope = 23.2** (radius + knockback + visual alignment together, since the three must agree by construction; add an assert tying visual radius to damage radius).

**(5) Bomb base damage ×2.** Verified levels `dmg` 40/52/66/82/100 with a monotonic ramp already in place → double the **base** (40→80) and preserve the existing scaling shape; test-logic already asserts `weapons.bombs: dmg↑ r↑ cd↓ fuse↓`, which continues to hold. **Scope = 23.2** (same step as #4 — one weapon, one tuning pass, one review).

**(6) Synergy visual identity for all synergized weapons.** Verified: synergies today are almost entirely **stat/DoT effects with no distinct projectile appearance** — `blight` adds DoT to bolts, `inferno` adds burn to bullets, `napalm` adds DoT to bomb blasts; nothing recolors or re-shapes the projectile itself. Only `tempest` spawns its own bolts. So this ask is real and currently unmet across the board. It requires **new procedural art** (per rule 2: no external assets — all in `js/art`), per-projectile variant sprites/tints, and likely a `variant`/`synergy` tag on projectile records so draw picks the right sprite. This is the largest of the six and overlaps the **visual upgrade pass the user designated as next session's priority** → **scope = Phase 24 (visual upgrade pass), step 24.x**, built on top of whatever sprite-variant seam that pass establishes, rather than bolting a bespoke hack per synergy now. Sequencing note: do #6 once with a shared mechanism, not six times.

### 3.15 Playtest defects round 4 + over-heal synergy + performance pass (user report 2026-09-04, second batch; **scoping-only — no implementation**) + workspace-hygiene process rule

**Process rule (binding, added to AGENTS.md non-negotiables this turn):** maintain a neat/tidy/efficient workspace **proactively** — delete accidentally-created strays (incl. Windows reserved-name files like `nul`), fix typos the moment they are noticed, and handle any similar hygiene issue in the same turn it appears, without being told.

**(1) Mobile "View zoom" reported non-functional (Pages build).** Investigated in-tree: the feature is **present and correct**, and IS in the published `main` (`git grep _applyZoom main` = 3 hits), so this is not missing code. Cleared each suspect: `_applyZoom()` grows the camera view by `1/zoom`; measured numerically at 0.8 the view rect gains **+56% area** across portrait (390×844) / landscape (844×390) / tablet — matching spec. The camera clamp does **not** defeat zoom (travel range stays ≈3100–3800 px). `#set-zoom` exists in `index.html`, is styled, and persists via `qsurv.zoom.v1`. Render applies the transform consistently across passes. **Real defect found:** `lighting.draw()` runs *inside* the zoomed view transform while its offscreen canvas is sized to the view (`resize()` → half-res of the view), so at 0.8 the darkness layer is stretched over a larger rect and lighting resolution degrades exactly when mobile users expect it to hold. **Needs a device-side repro** (cannot run a phone here) to determine what "not functioning at all" means — likely either the Settings row is not reachable/visible on the user's device, or the effect is too subtle against an enlarged view. → **22.8**.

**(2) Over-heal synergy — DECIDED 2026-09-04: REPLACE Phoenix Heart's effects in place; do NOT re-home/relocate the old kill-heal effect.** Current in-tree definition being replaced: `phoenix` = "Reclaim your vitality — heal 10 HP every 8 kills", `requires: ['hp','regen']`, **1 level** `{heal: 10, every: 8}`; runtime `game.js:508` increments `_phoenixKills` and calls `p.heal()` on every 8th kill, where `heal()` is clamped (`Math.min(maxHp, hp+n)`) so it does nothing at full health. **Replacement spec (user, verbatim intent):** hearts picked up **at full health** grant over-healing past 100% of max HP, up to a ceiling of **200% of the player's CURRENT max HP**; the bonus diminishes **1%/s** back down to 100%; picking up another heart momentarily cancels/nullifies/reverses that diminishment and re-applies over-health for the amount a heart would normally heal an injured player (`CFG.gems.heartHeal`), never exceeding the 200% ceiling. **Implementation consequences (verified):** (a) `Pickups.update` currently returns `{xp, heal}` and the game blind-calls `pl.heal(got.heal)` — hearts have **no magnet logic at all** today (only gems test `d < magR`), so this synergy needs a new player field (e.g. `overHeal`) + a per-step decay tick + a non-clamped heal path; (b) the 200% ceiling is of **current** max HP, so it must be re-evaluated if max HP changes mid-run (Heart of Oak picks); (c) HUD must show over-health or players read it as "nothing happened" — pairs with Phase 19's HUD work; (d) the old kill-counter machinery (`_phoenixKills` in `player.js:71`, `game.js:508–511/714`, card-text fn `player.js:414`) is **deleted, not moved** per the user's explicit instruction. **Test blast radius (important):** `phoenix` is currently the **canonical passive-pair synergy fixture** across the harness — `test-logic` uses it as the gating/pool/cap assertion in ≥5 places (`cap 4 → pool = phoenix only`, 3-candidate pool, etc.) and `test-boot` uses it for the 10.7 empty-pool guard and the 11.6b ownership E2E. Those assertions key on the **key** `phoenix`, so they survive an effects swap, but any that assert its card text or level count must be rebaselined; the new over-heal behaviour needs its own invariants (ceiling = 2× current max HP, 1%/s decay, second-heart re-application, no over-heal below full HP). → **Phase 23.5**.

**(3) Framerate decays precipitously over a run, worse on mobile — user requires a DEDICATED FULL SESSION. APPROVED (decision b): new Phase 25, placed ahead of 10.9.** Verified: most load is **capped**, so the classic unbounded-leak story does not obviously hold — enemies `aliveCap = min(200, 60 + t/12)`, gems/hearts are fixed-size ring buffers (`maxAlive` 320 / `heartPool` 32), particles `particleCap` 512. Two concrete contributors identified: sustained Pyre Lance emission of **2–3 sprites every frame** while any target is in range (never idles), and the 22.2 `escapeFromSpots` per-gem-per-frame nudge (up to 8 guard iterations × spots × up to 320 gems, every step). Phase 25 must **profile first** (the existing `[10.4-bench]` harness is the right instrument; sample a long run at fixed intervals) rather than guess — same measure-first discipline that caught the 16.1 snow regression (D70). Merge/absorb **10.9** (the existing bench/optimization step, already LAST per D25) into Phase 25 so the two don't compete.

**(4) 22.6 death→Game Over delay — DONE 2026-09-04: the slow-motion death beat was REMOVED.** Root found (no repro needed): `CFG.run.deathDelay = 1.8` s with `deathTimescale = 0.3` made death enter `DYING`, drop timescale to 0.3 (slow-mo), and only call `_gameOver(false)` once `deathT >= 1.8`. At 0.3× that reads as a long pause before the Game Over screen — i.e. an intentional slow-motion death beat, not a bug. The user's 2026-09-04 verdict: **no clear visual indicator for the slow-mo (no dying animation) → the beat is useless → remove it.** Implementation: deleted both scalars from `config.js` (`CFG.run.deathDelay`/`deathTimescale`); `_onDeath` in `game.js` now does `camera.addShake(1)` → `bus.emit('death')` → `this._gameOver(false)` with no `DYING` transient (death shake + `death` bus retained; `_gameOver` already resets `timescale=1`). Now death transitions **immediately** to `GAMEOVER`. Cleaned the dead `this.deathT` field inits (game.js:112/183/828), dropped `DYING` from the HUD-show condition (hud.js), and re-based test-boot's death assert to check `GAMEOVER` directly. → **closed**.

**Where each lands (best-practice placement):**
- Defect-class items in the **open Phase 22**: **(1) view-zoom → `22.8`** (device repro needed); **(4) death-delay → `22.6`** (DONE 2026-09-04 — slow-mo death beat removed; death now ends the run immediately).
- Over-heal synergy **(2)** → **`23.5`** (Phase 23 — it is a config-table + heal-pipeline change in the same subsystem Phase 23 already touches, so one review pass covers both).
- Performance pass **(3)** → **new Phase 25**, dedicated full session, placed **ahead of 10.9** and absorbing it (approved by user 2026-09-04).

### Legacy placement (previous batch — §3.14)
- Defect-class items in the **open Phase 22** (playtest defects round 2 + 3): **(1) flame-range alignment → `22.5`** (DONE 2026-09-04, flat 150); **view-zoom → `22.8`** (device repro needed).
- Weapon tuning + new mechanic → **new Phase 23 — projectile feel & bomb rework**, inserted after Phase 22 and before 12.3 (it touches the same config tables and card-text/gate tests that 12.3–12.8 will extend; doing it first avoids rebaselining those asserts twice).
