...
export class World {
  ...
  generate(seed) {
    ...
    this.lakes = d.lakes.map((l) => ({ ...l, img: lakeSprite(mulberry32(l.ls), l.rx, l.ry) }));
  ...
  drawBackground(ctx, cam, t) {
    ...
    // moon, top-left
    ctx.drawImage(sky.moon, vw * 0.06, horizon * 0.22);
    ...
    for (let ty = (y0 / T) | 0; ty * T < y1; ty++) {
      for (let tx = (x0 / T) | 0; tx * T < x1; tx++) {
        ...
        ctx.drawImage(terrain.grassTiles[(h * 3) | 0], tx * T, ty * T);
      }
    }
    ctx.globalAlpha = 1;
